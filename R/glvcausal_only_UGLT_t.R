glvcausal_only_UGLT_t <- function(data, mh_setup_lst, init_lst, prior_lst, chain_setup_lst, verbose){
  ...
  if(is.null(prior_lst$nu_t)) stop("prior_lst$nu_t is required for glvcausal_only_UGLT_t")
  ...
  for(it in 2:chain_setup_lst$Nit){
    ...
    # Use update_P_star_t for Student-t
    ...
    if(verbose){
      if(it %% 1000 == 0){
        cat("Iteration: ", it, "/", chain_setup_lst$Nit, " completed for second step \n")
      }
    }
