...
update_P_star_split <- function(mu, A, B, L, C, sigma2, a1, a2, sparsity_matrix, pivot, P_star, kappa, mcmc_setup_lst, prior_lst, data){
  ...
  # New sigma2
  # (8*sigma2_new[pivot_propose] - U^2*8*sigma2_new[pivot_propose])/8
  ...
  # New loading
  l_propose <- U*sqrt(8*sigma2[pivot_propose])
  L_propose <- rep(0, Q)
  ...
  loglik_old <- compute_loglik_rjmcmc_cpp(pivot_propose, B, A, L, C, mu, sigma2, Y, X)
  loglik_new <- compute_loglik_rjmcmc_cpp(pivot_propose, B, A, L_new, C_new, mu, sigma2_new, Y, X)
  ...
  # Jacobian
  log_jacobian <- log(sqrt(8)) + log(sqrt(sigma2[pivot_propose]))
...
update_P_star_merge <- function(mu, A, B, L, C, sigma2, a1, a2, sparsity_matrix, pivot, P_star, kappa, mcmc_setup_lst, prior_lst, data){
  ...
  # New sigma2
  sigma2_new <- sigma2
  sigma2_propose <- sigma2[pivot_merge] + (L[pivot_merge,column_merge]^2)/8
  ...
  # log likelihood ratio
  loglik_old <- compute_loglik_rjmcmc_cpp(pivot_merge, B, A, L, C, mu, sigma2, Y, X)
  loglik_new <- compute_loglik_rjmcmc_cpp(pivot_merge, B, A, L_new, C_new, mu, sigma2_new, Y, X)
  log_lik_ratio <- loglik_new - loglik_old
  ...
  # Jacobian
  log_jacobian <- -(log(sqrt(8)) + log(sqrt(sigma2_new[pivot_merge])))
